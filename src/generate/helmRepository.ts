...
export const getHelmRepositoryURL = async (
  ...
  const yamlFiles = await recursiveReadDir(repositoryPath, [
    filterYamlFilesFunc,
  ]);
  ...
  const helmRepositoryURL = helmRepositories.find(
    (h) => h["metadata"]["name"] === helmRepository
  )["spec"]["url"];
